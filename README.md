...
### 📝 ASMR 时间轴支持
- 从 ASMR 站点提取时间轴章节（支持 div#chapter / div.chapter / tab_content）
- 日文 / 韩文章节标题自动翻译为中文（deep-translator Google 翻译）
- 章节信息附带在 HLS 资源上（可扩展嵌入 ID3 同步歌词）
...
### 2. 下载 HLS 音频（ASMR）
输入 ASMR 站点 URL → 抓取 → 自动检测到 HLS 流并勾选 → 下载 → 自动合并为完整音频文件。

### 3. 批量下载多页面
用逗号或换行分隔多个 URL，一次抓取多个页面的资源。

### 4. 翻译日文章节
抓取含时间轴章节的 ASMR 页面后，章节标题自动翻译为中文（需要网络连接 Google 翻译）。
...
2. **章节嵌入需扩展**：Resource 对象携带了 chapters 数据，但尚未实现 mutagen ID3 嵌入，可按需扩展 `core/downloader.py`
3. **反爬站点**：部分站点有反爬机制，建议降低并发数（设置 → 并发下载 → 1~4）
